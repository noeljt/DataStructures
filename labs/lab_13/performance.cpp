...
void list_test(const std::string* input_data, int input_count, const std::string &operation, std::string *output_data, int &output_count) {
 ...
 // for all cases, simply put the data into a list
  std::list<std::string> lis;
  for (int i = 0; i < input_count; i++) {
    lis.push_back(input_data[i]);
  }
  if (operation == "sort") {
    // use the sort algorithm
    lis.sort();
    output_count = input_count;
    std::list<std::string>::iterator it = lis.begin();
    for (int i = 0; i < output_count; i++, it++)
      output_data[i] = *it;
  } 
  else if (operation == "remove_dups") {
    // don't reorder the elements, just do all pairwise comparisons
    ...
    int mode_count = 0;
    // keep track of two pointers into the structure
    std::list<std::string>::iterator current = lis.begin();
    ++current;
    std::list<std::string>::iterator previous = lis.begin();
  ...
  // if they are the same increment the count
    ...
    // save the mode to the output listor
    output_count = 1;
    output_data[0] = mode;
  } else {
    std::cerr << "Error: Unknown operation: " << operation << std::endl;
...
void hash_table_test(const std::string* input_data, int input_count, const std::string &operation, std::string *output_data, int &output_count) {
  ...
  if (operation == "sort") {
    std::cout << "Dumb thing to do with hash tables." << std::endl;
  } else if (operation == "remove_dups") {
    std::unordered_set<std::string> ht(input_count);
    for (int i=0; i<input_count; i++) {
      if (ht.insert(input_data[i]).second) {
        output_data[output_count] = input_data[i];
        output_count++;
      }
    }
    ...
    for (int i=0; i<input_count; i++) {
      ht[input_data[i]]++;
      if (ht[input_data[i]]>current_value) {
        current_value = ht[input_data[i]];
        current_key = input_data[i];
      }
    }
    output_data[0] = current_key;
    output_count = 1;
  } else {
    std::cerr << "Error: Unknown operation: " << operation << std::endl;
    usage();
