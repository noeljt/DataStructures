...
public:
  ...
  friend void deleteAll(std::set<Balloon*> & balloons, Balloon* & b, bool first);
private:
  std::string name;
  int num_ropes;
...
// ====================================================
void deleteAll(std::set<Balloon*> & balloons, Balloon* & b, bool first);
void deleteAll(Balloon* & b) {
  std::set<Balloon*> balloons;
  balloons.insert(b);
  deleteAll(balloons,b,true);
}
void deleteAll(std::set<Balloon*> & balloons, Balloon* & b, bool first) {
  if (!b) return;
  for (int i=0; i<b->numRopes(); i++) {
    if (balloons.find(b->ropes[i])==balloons.end()) {
      balloons.insert(b->ropes[i]);
      deleteAll(balloons,b->ropes[i],false);
    }
  }
  if (first) {
    std::set<Balloon*>::iterator it = balloons.begin();
