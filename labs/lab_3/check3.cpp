...
void change_time(Time & t1, Time t2)
{
  ...
  std::cout << "change_time() address: "; //<< &change_time << std::endl;
  std::cout << "time 1 address: " << &t1 << std::endl;
  std::cout << "time 2 address: " << &t2 << std::endl;
  //label_stack(,"main");
  //label_stack(,"change_time");
  label_stack((uintptr_t*)&t1, "time 1");
  label_stack((uintptr_t*)&t2, "time 2");
  print_stack();
  clear_stack_labels();
...
int main() {
  std::cout << "Bytes for Time Class: " << sizeof(Time) << '\n';

  Time low(0,0,0);
  Time high(12,12,12);

  label_stack((uintptr_t*)&low,"low");
  label_stack((uintptr_t*)&high,"high");
  change_time(low,high);
}
