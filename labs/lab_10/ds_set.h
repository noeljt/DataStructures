...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    if (old_root->left!=NULL) {
      TreeNode<T>* left = new TreeNode<T>(old_root->left->value);
      new_root->left = left;
      left->left = old_root->left->left;
      left->right = old_root->left->right;
      copy_tree(left);
    }
    if (old_root->right!=NULL) {
      TreeNode<T>* right = new TreeNode<T>(old_root->right->value);
      new_root->right = right;
      right->left = old_root->right->left;
      right->right = old_root->right->right;
      copy_tree(right);
    }
    return new_root;
  }
