...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    if (p == m_table[index].end()) {
      ...
      m_size++;
      return std::make_pair(iterator(this,index,--m_table[index].end()),true);
    }
    else 
      return std::make_pair(iterator(this,index,p),false);    
  }
  ...
  iterator begin() {
    ...
    for (++p.m_index; 
         p.m_index < int(p.m_hs->m_table.size()) && p.m_hs->m_table[p.m_index].empty();
         ++p.m_index) {}
    
    // If one is found, assign the p.m_list_itr to the start
    if (p.m_index != int(p.m_hs->m_table.size()))
      p.m_list_itr = p.m_hs->m_table[p.m_index].begin();
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    m_table = new_table;
    for (typename std::vector<std::list<KeyType> >::iterator it=old_table.begin();
                  it!=old_table.end(); it++) {
      for (typename std::list<KeyType>::iterator it2=(*it).begin();
                    it2!=(*it).end(); it2++) {
        this->insert(*it2);
      }
    }
