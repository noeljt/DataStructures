...
public:
  ...
  /*
    //trivial w/ many collisions
    unsigned int hash = 0;
    hash += key.length()%key.size();
    return hash;
  }
...
int main() {
  ...
  //student test of anagram
  /*
  insert_result = set1.insert( std::string("endfri")    );
  assert( insert_result.second );

  insert_result = set1.insert( std::string("dnefri")    );
  assert( insert_result.second );

  insert_result = set1.insert( std::string("eindfr")    );
  assert( insert_result.second );
  */
