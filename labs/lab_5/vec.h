...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T> & v, const T& element)
{
  typename Vec<T>::size_type num = 0;
  typename Vec<T>::iterator i;
  bool last = false;
  ...
  if (last==false)
  {
    v.resize(v.size(), element);
  }
  else
  {
    v.resize(v.size()-1, element);
  }
  return num;
}
//O(N+N*E)
