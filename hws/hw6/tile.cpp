...
Tile::Tile(std::string &north, std::string &east,
           std::string &south, std::string &west) :
...
Tile* Tile::rotate(int rot) {
  Tile* t;
  if (rot==90) {
    t = new Tile(west_,north_,east_,south_);
    return t;
  }
  else if (rot==180) {
    t = new Tile(south_,west_,north_,east_);
    return t;
  }
  else if (rot==270) {
    t = new Tile(east_,south_,west_,north_);
    return t;
  }
...
void Tile::printRow(std::ostream &ostr, int row) {
...
void Tile::prepare_ascii_art() {
  ...
  //ruct the road fragments from edge towards the center of the tile
  if (north_ == "road") {
  ...
  //ruct the curved pieces of "corner" roads
  if (!center_road) {
  ...
  // -------------------------------------------------------------------
  // CITIES
  //ruct the curved wedges of cities for each edge
  if (north_ == "city") {
